(function(global){
    ...
    this.merge=typeof option.merge==="boolean"?option.merge:true;this.TimetableType=option.timetableType||[];
    this.leftHandText=[];this.highlightWeek=option.highlightWeek||"";
    ...
    this.palette=(typeof styles.palette==="boolean"&&!styles.palett)?false:(styles.palette||[]).concat(this.defaultPalette);
        this._init()};Timetables.prototype={
            ...
            _init:function(option){
                ...
                leftHand.style.left=0;leftHand.style.top=0;leftHand.style.width=leftHandWidth+"px";
                var timetable=Timetables[0].map(function(v,i){return[]});
                timetable.forEach(function(item,index){
                    Timetables.forEach(function(val,i){timetable[index].push(val[index])})
                });
                var listMerge=[];
                if(merge){
                            ...
                            if(item===(listMerge[i][index-1]||{}).name&&item){
                                ...
                                if(sameIndex||sameIndex===0){
                                    ...
                                    return listMerge[i].push({
                                        name:item,length:0,sameIndex:sameIndex
                                    })
                                }
                                listMerge[i][index-1].length++;
                                return listMerge[i].push({
                                    name:item,length:0,sameIndex:index-1
                ...
                weekItem.innerText=item;weekItem.style.cssFloat="left";
                weekItem.style.boxSizing="border-box";weekItem.style.whiteSpace="nowrap";
                weekItem.style.width=100/week.length+"%";head.appendChild(weekItem)});
                ...
                var paletteIndex=0;timetable.forEach(function(values,index){
                    ...
                    values.forEach(function(item,i){
                        ...
                        courseItem.style.boxSizing="border-box";courseItem.style.position="relative";
                        if(merge&&listMerge[i][index].length>1){
                            var mergeDom=document.createElement("span");
                            mergeDom.style.position="absolute";
                            mergeDom.style.zIndex=9;mergeDom.style.width="100%";
                            ...
                            if(palette){
                                ...
                                mergeDom.style.color="#fff";paletteIndex++;
                                if(paletteIndex>palette.length){
                                    paletteIndex=0
                                }
                            }
                            mergeDom.innerText=listMerge[i][index].name;mergeDom.className="course-hasContent";
                                        ...
                                        deepCopyTimetableType.forEach(function(item,index){
                                            ...
                                            if(typeof item[0]==="object"){
                                                for(var v in item[0]){
                                                    ...
                                                    handItemInner.style.margin="0px";handItemInner.className="left-hand-"+v;
                ...
                (option||{}).setNewOption=true;this._init(option)
            }
        };
        ...
        if(typeof define==="function"){
            define(function(){return Timetables})
        }
        global.Timetables=Timetables})(this);
